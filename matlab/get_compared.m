...
%get_compared Сравнение заданного векторного поля с результатом обработки
%   Находит среднеарифметическое модуля отклонения двух полей и максимум отклонения по одной из координат 

% Обязательно указать откуда брать сравниваемое векторное поле
if isempty(varargin), error('Не заполнены параметры'); end
...
if flow % в сравнении с полем из файла flow
    ...
    if ~isequal(size(vectors_map),size(result_map)), error('Не соответствие размеров'); end
