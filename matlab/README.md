...
Среднеарифметическое модуля отклонений рассчитывается по следующей формуле: <br>
$$ \displaystyle\ mean = \frac{\sum [|x-x_0|+|y-y_0|]}{2\times H\times W} $$ <br>
...
Среднеквадратическое отклонений рассчитывается по следующей формуле: <br>

$$ \displaystyle\ RMS = \sqrt\frac{\sum [(x-x_0)^2+(y-y_0)^2]}{H\times W (H\times W - 1)} $$ <br>
где `H`, `W` – размеры векторного поля.
